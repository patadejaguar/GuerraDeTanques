    ...
    :ITank(tank_info, nick, rank, color_team)
{
    ...
    _animation_whell = getListOfPixmapFromStripImage(":/xavier_tank_pantanoso/sprites/whell.png",18);
    _body = QPixmap(":/xavier_tank_pantanoso/sprites/body.png");
    _turret = QPixmap(":/xavier_tank_pantanoso/sprites/turret.png");
...
void Tank::setCurrentLive(double current_live, ITank *tank_cause_change, bool animate_numbers)
{
    if(_skill_3_is_activated && tank_cause_change != this && _enemie.contains(tank_cause_change) && !_tanks_for_skill_3.contains(tank_cause_change))
    {
        _tanks_for_skill_3.append(tank_cause_change);
        tank_cause_change->setCurrentLive(tank_cause_change->currentLive()-(_current_live - current_live)*2, this);
    }
    _tanks_for_skill_3.removeAll(tank_cause_change);
    ITank::setCurrentLive(current_live, tank_cause_change, animate_numbers);
}

void Tank::setActivateSkill3(bool value)
{
    _skill_3_is_activated = value;
    if(!value)
        _tanks_for_skill_3.clear();
}
