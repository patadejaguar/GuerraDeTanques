...
Skill1::Skill1(ITank *tank, int damage, int time_effect, QPointF turret_pos, int turret_angle, QList<QPixmap> pixmap_shot) :
    ISkill(tank)
{
    _damage = damage;
    ...
    _sound = new SoundEngine(QUrl("qrc:/xavier_tank_pantanoso/sounds/shot_explode.mp3"), MECH_MAX_DISTANCE_SOUND, this);
}
        ...
        {            
            ...
            else if(enemy->getStatus() != DEAD && collidesWithItem((QGraphicsItem*)enemy))
            {
                ...
                if(_damage > enemy->getTotalArmor())
                {
                    ...
                    Effect* effect = new Effect(_tank, enemy, 0, 0, 0, -3, false, false, _time_effect);
...
Skill1Factory::Skill1Factory(ITank *tank, SkillInfo skill_info):
    ISkillFactory(tank, skill_info, 1200, 10)
{
    _damage = 25;

    _time_effect = 3;

    _sound_engine = new SoundEngine(QUrl("qrc:/xavier_tank_pantanoso/sounds/shot.mp3"), MECH_MAX_DISTANCE_SOUND, this);

    _pixmaps_shot = getListOfPixmapFromStripImage(":xavier_tank_pantanoso/sprites/skill_1.png",24);
}
...
QString Skill1Factory::getDetails()
{
    ...
    if(isActive())
    {
        QString details = QString("<table>"
                                  ...
                                  "</tr>"
                                  "<tr>"
                                  "<td><p class=\"w\" align=\"justify\">Ralentiza: </p></td>"
                                  "<td><p class=\"yl\" align=\"justify\">3 por %2 seg</p></td>"
                                  "</tr>"
                                  "<tr>"
                                  ...
                                  "</tr>"
                                  "</table>").arg(_damage).arg(_time_effect).arg(_mana_cost).arg((double)_cooldown/1000,0,'f',1);
...
ISkill *Skill1Factory::createSkill()
{
    ...
    //creo la habilidad
    ISkill* skill = new Skill1(_tank, _damage, _time_effect, turret_pos, angle, _pixmaps_shot);

    //efecto de fuego.
    SingleAnimation *_shot_fire = new SingleAnimation(turret_pos,angle, getListOfPixmapFromStripImage(":xavier_tank_pantanoso/sprites/shot_fire_skill_1.png",30));
