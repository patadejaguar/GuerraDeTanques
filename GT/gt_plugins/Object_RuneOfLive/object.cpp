...
Object::Object(bool edit_mode):IObject(edit_mode)
{
    ...
    _tileset = getListOfPixmapFromStripImage(":/xavier_object_rune_of_live/rune_of_live.png",_width);

    _explode = getListOfPixmapFromStripImage(":/xavier_object_rune_of_live/explode.png",60);

    _mini_map_view->setPixmap(QPixmap(":/xavier_object_rune_of_live/mini_map_object.png"));

    _sound = new SoundEngine(QUrl("qrc:/xavier_object_rune_of_live/rune.mp3"), MECH_MAX_DISTANCE_SOUND, this);

    _effect_animation = getListOfPixmapFromStripImage(":/xavier_object_rune_of_live/effect_animation.png",85);
}
...
QList<QWidget *> Object::getControls()
{
    ...
    Control *c_bonus = new Control("Bonificación de vida", bonus);
...
void Object::gameLoop()
{
    ...
    else
    {
        ...
        foreach(ITank* tank, _tanks)
        {
            ...
            {
                Effect* effect_tank = new Effect(tank, tank, _bonus, 0, 0, 0, false, false, _time, _effect_animation, -0.1);
