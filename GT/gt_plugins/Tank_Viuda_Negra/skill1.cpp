...
Skill1::Skill1(ITank *tank, int damage, int slow_down, QPointF turret_pos, int turret_angle, QList<QPixmap> pixmap_shot) :
    ISkill(tank)
{
    _damage = damage;
        ...
        {            
            ...
            else if(enemy->getStatus() != DEAD && collidesWithItem((QGraphicsItem*)enemy))
            {
                ...
                if(_damage > enemy->getTotalArmor())
                {
                    ...
                    int percent = getPart(_tank->getTotalSpeed(),_slow_down);
                    Effect* effect = new Effect(_tank, enemy, 0, 0, 0, -percent, false, false, 4);
...
Skill1Factory::Skill1Factory(ITank *tank, SkillInfo skill_info):
    ISkillFactory(tank, skill_info, 1100, 10)
{
    _damage = 38;
    ...
    _pixmaps_shot = getListOfPixmapFromStripImage(":xavier_tank_viuda_negra/sprites/skill_1.png",24);
}
...
QString Skill1Factory::getDetails()
{
    ...
    if(isActive())
    {
        QString details = QString("<table>"
                                  ...
                                  "</tr>"
                                  "</table>").arg(_damage).arg(_slow_down).arg(_mana_cost).arg((double)_cooldown/1000,0,'f',1);
...
ISkill *Skill1Factory::createSkill()
{
    ...
    //creo la habilidad
    ISkill* skill = new Skill1(_tank, _damage, _slow_down, turret_pos, angle, _pixmaps_shot);

    //efecto de fuego.
    SingleAnimation *_shot_fire = new SingleAnimation(turret_pos,angle, getListOfPixmapFromStripImage(":xavier_tank_viuda_negra/sprites/shot_fire_skill_1.png",30));
