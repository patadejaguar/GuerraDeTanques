...
Skill1Factory::Skill1Factory(ITank *tank, SkillInfo skill_info):
    ISkillFactory(tank, skill_info, 1000, 20)
{
    _damage = 40;

    _sound_engine = new SoundEngine(QUrl("qrc:/xavier_tank_nostradamus/sounds/shot.mp3"), MECH_MAX_DISTANCE_SOUND, this);

    _pixmaps_shot = getListOfPixmapFromStripImage(":xavier_tank_nostradamus/sprites/skill_1.png",24);
}
...
ISkill *Skill1Factory::createSkill()
{
    ...
    //efecto de fuego.
    SingleAnimation *_shot_fire = new SingleAnimation(turret_pos,angle, getListOfPixmapFromStripImage(":xavier_tank_nostradamus/sprites/shot_fire_skill_1.png",30));
